...
<div class="bg-white rounded-lg shadow overflow-hidden">
    <div class="px-6 py-4 border-b border-gray-200">
        <h2 class="text-lg font-medium text-gray-900">Create Page</h2>
    </div>
    <form action="{{ route('pages.store') }}" method="POST" class="p-6">
        @csrf
        <div class="grid grid-cols-1 gap-6">
                ...
                <div>
                    ...
                    <select name="category_id" id="category_id" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm border p-2">
                        ...
                        @foreach($categories as $category)
                            <option value="{{ $category->id }}" {{ old('category_id') == $category->id ? 'selected' : '' }}>{{ $category->title_uz }}</option>
                        @endforeach
                    </select>
                </div>
            ...
            <div>
                ...
                <textarea name="content_uz" id="content_uz">{{ old('content_uz') }}</textarea>
            </div>

            <div>
                <label for="content_en" class="block text-sm font-medium text-gray-700 mb-2">Content (EN)</label>
                <textarea name="content_en" id="content_en">{{ old('content_en') }}</textarea>
            </div>
